...
function getApiHost(): string {
  const base = getApiBase();
  if (!base) return '';
  // Strip /api/v1 or trailing slash so we can append /api/v1 consistently
  return base.replace(/\/(api\/v1?)?\/?$/, '').replace(/\/$/, '');
}

/** URL to start Google OAuth. Direct link to /auth/google_oauth2 (no /api/v1). */
...
export async function request<T>(
  ...
  let res: Response;
  try {
    res = await fetch(`${baseUrl.replace(/\/$/, '')}${path.startsWith('/') ? path : `/${path}`}`, { ...init, headers });
  } catch (e) {
    ...
    const msg = isNetworkError
      ? `Cannot reach the API at ${getApiBase() || baseUrl}. Check VITE_API_URL (or meta api-url) and that the Rails server is running.`
  ...
  if (!res.ok) {
    ...
    if (res.status === 404) {
      ...
      if (hasJsonError) throw new Error(msg);
